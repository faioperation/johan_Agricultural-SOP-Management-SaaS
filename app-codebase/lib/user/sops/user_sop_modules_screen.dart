...
import 'package:farm_check_support/user/sops/milking_sop_list_screen.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class UserSopModulesScreen extends StatefulWidget {
  const UserSopModulesScreen({super.key});

  @override
  State<UserSopModulesScreen> createState() => _UserSopModulesScreenState();
}

class _UserSopModulesScreenState extends State<UserSopModulesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        if (_sopController.isLoadingModules.value) {
          return const Center(child: CircularProgressIndicator());
        }

        if (_sopController.sopModules.isEmpty) {
          return const Center(child: Text("No SOP modules found"));
        }

        return RefreshIndicator(
          onRefresh: () => _sopController.fetchSopModules(),
          child: SingleChildScrollView(
            physics: const AlwaysScrollableScrollPhysics(),
            padding: EdgeInsets.all(w * 0.04),
            child: Column(
              children: [
                ...List.generate(_sopController.sopModules.length, (index) {
                  final module = _sopController.sopModules[index];
  ...
  Widget _sopItem(
    ...
    required String imagePath, // 👈 PNG image
    ...
    return InkWell(
      ...
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => MilkingSopListScreen(moduleName: title),
          ),
        );
      },
      child: Container(
