...
import 'package:farm_check_support/user/messages/user_chat_screen.dart';
...
class UserMessagesScreen extends StatelessWidget {
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _messageCard(context,
                        w: w, conv: conv, controller: controller);
                  },
                ),
              );
            }),
  ...
  Widget _messageCard(BuildContext context, {required double w, required ConversationModel conv, required UserChatController controller}) {
        ...
        Navigator.of(context, rootNavigator: true).push(
          MaterialPageRoute(
            builder: (_) => UserChatScreen(
              userId: conv.userId,
              userName: conv.name,
              jobTitle: conv.role ?? conv.jobTitle ?? "Manager",
            ),
          ),
        );
      },
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.chat_bubble_outline, color: Colors.blue, size: w * 0.06),
            ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        conv.name,
                        style: TextStyle(fontWeight: FontWeight.w600, fontSize: w * 0.035),
                      ),
                      Row(
                        children: [
                          Text(
                          ...
                          if (conv.unreadCount > 0) ...[
                            ...
                            Icon(Icons.circle, size: w * 0.02, color: Colors.green),
                          ],
                        ],
                      ),
                    ],
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: w * 0.032, color: Colors.black87),
                  ),
                  SizedBox(height: 10),
                  Container(
                    padding: EdgeInsets.symmetric(horizontal: w * 0.03, vertical: w * 0.01),
                    decoration: BoxDecoration(
                      color: (conv.role == "MANAGER" || conv.jobTitle == 'Manager')
                          ? const Color(0xFFFFEFEA)
                          : const Color(0xFFEFF4FF),
                      borderRadius: BorderRadius.circular(20),
                    ),
                    child: Text(
                      conv.role ?? conv.jobTitle ?? "Manager",
                      style: TextStyle(
                        fontSize: w * 0.028,
                        color: (conv.role == "MANAGER" || conv.jobTitle == 'Manager') ? Colors.orange : Colors.blue,
